...
DATA_DIR = "/app/data"  # папка в контейнере, смонтированная на хосте
os.makedirs(DATA_DIR, exist_ok=True)  # создаем, если нет
...
class Record:  # class for working with records
    ...
    def add_birthday(
        self, birthday
    ):  # add birthday to the record self.birthday = add_birthday
    ...
    def get_upcoming_birthdays(self, days=7):  # get upcoming birthdays
        ...
        for contact_b in self.data.values():
            ...
            birthday_date = datetime.strptime(
                contact_b.birthday.value, "%d.%m.%Y"
            ).date()
            ...
            if 0 <= (birthday_this_year - today).days <= days:
                ...
                upcoming_birthdays.append(
                    {"name": contact_b.name.value, "birthday": congratulation_date_str}
                )
...
def birthdays(book: AddressBook):  # get upcoming birthdays
    ...
    return "\n".join(
        f"{item['name']} - congratulations day: {item['birthday']}" for item in result
    )
...
def load_data(filename="addressbook.pkl"):
    ...
    try:
        ...
        print("Адресна книга не знайдена. Буде створена нова.")
...
def main():
    ...
    while True:
        user_input = input(
            "Enter a command (For help, type 'h' or '?'): "
